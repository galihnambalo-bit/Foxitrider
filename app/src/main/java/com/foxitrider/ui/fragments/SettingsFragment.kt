...
class SettingsFragment : Fragment() {
        ...
        b.btnUnlockAll.setOnClickListener {
            if (AdManager.isRewardedReady()) {
                AdManager.showRewarded(requireActivity(),
                    onRewarded = {
                        ProFeatureManager.unlockAll(requireContext())
                        updateProStatus()
                        Toast.makeText(context, "🎉 Semua fitur Pro dibuka!", Toast.LENGTH_LONG).show()
                    },
                    onFailed = {
                        Toast.makeText(context, "Iklan tidak tersedia", Toast.LENGTH_SHORT).show()
                    })
            } else {
        ...
        b.btnShare.setOnClickListener {
            startActivity(Intent.createChooser(
                Intent(Intent.ACTION_SEND).apply {
                    type = "text/plain"
                    putExtra(Intent.EXTRA_TEXT, "Coba Foxit Rider - PDF Editor gratis!")
                }, "Bagikan"
            ))
        }
    }
    ...
    private fun updateProStatus() {
        ...
        b.tvProStatus.text = if (ms > 0) {
            "✅ Pro aktif: ${ms / 3600000}j ${(ms % 3600000) / 60000}m"
        } else {
            "⭐ Tonton iklan untuk buka fitur Pro"
        }
    }
